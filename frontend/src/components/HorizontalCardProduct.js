...
import fetechcategoryByProduct from "../helpers/fetchCategorywiseProduct";
...
const HorizontalCardProduct = ({ category, heading }) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const categoryProduct = await fetechcategoryByProduct(category);
    setLoading(false);
      ...
      >
        ...
        {loading
          ...
          : data.map((product, index) => {
              return (
                <Link to={"product/" + product?._id} key={index} className="w-full md:min-w-[300px] min-w-[280px] md:max-w-[300px] max-w-[280px] h-36 bg-white rounded-sm shadow-md flex">
                  ...
                  <div className="p-4 capitalize grid ">
                    ...
                    <div className="flex gap-2">
                      <p className="text-red-600 font-medium">
                        {displayJPCurrency(product?.sellingPrice)}
                      </p>
                      <p className="text-slate-500 line-through">
                        {displayJPCurrency(product?.price)}
                      </p>
                    </div>
                    <button className="text-sm bg-red-400 hover:bg-red-500 text-white px-3 py-1 rounded-md" onClick={(e)=>handleAddToCart(e,product?._id)}>
                      Add To Cart
                    </button>
                  </div>
                </Link>
